<script>
  ...
  const changeState = () => {
    if (browser) {
      ...
      else if (category === "shoes") positions = { ...positions, shoes: true };
      else if (category === "jewelry") positions = { ...positions, jewelry: true };
    ...
    {#each array as element, i}
      <li class="page-item" class:active={i === index - 1}>
        <a
          ...
          href={`${color === "" ? `${category}?i=${i + 1}` : `?color=${color}`}`}>{i + 1}</a>
      </li>
    {/each}
  </ul>
</nav>
